...
export const WorkshopNavigation = React.forwardRef<HTMLElement, WorkshopNavigationProps>(
  function WorkshopNavigation(props, forwardedRef) {
    ...
    React.useEffect(() => {
      ...
      if (listRef.current) {
        const activeLink = listRef.current.querySelector('a[aria-current="page"]');
        if (activeLink) {
    ...
    return (
      <nav className={clsx('text-base lg:text-sm', props.className)} ref={forwardedRef}>
                      ...
                      <li key={item.url} className="relative">
                        <a
                          ...
                          className={clsx(
                            ...
                            isActive
                              ? [
                                  'text-sky-600 before:bg-sky-500',
                                  !isLoaded && styles.sidebarHighlighted,
                                ]
