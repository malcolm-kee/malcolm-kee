...
There are tons of articles with click-baity titles like "Framework Battle" available today, as choosing frontend frameworks is an unsolved question that frontend engineers will face when evaluating their current projects.

However, there is not much being discussed about how to do the migration when you decided that you want to migrate to another framework.

In this post, I would like to share my previous experience driving migration of large Angular applications to React, and some of the lessons learnt that (in my opinion) are applicable to migration projects in general.

## Why Migrate to React

The tech stack used by the company was based on modern and popular technologies, and in general were maintained for a few years when I joined the company. On frontend, two frontend frameworks were used: Angular (2 web apps) & React (1 web app).
...
Unfortunately, the migration of the large Angular app was not done until the day I left the company. 
...
3. Invest in automated progress tracking. Automated progress tracking is important as it provides instant feedback to whoever works on the migration. This is somehow related to the item 2 above, the "React in Angular" approach makes the migration tracking challenging as legacy code in Angular and new code in React are interleaved together. If we went with the base href approach, progress tracking can be simply counting the number of files of Angular code.
