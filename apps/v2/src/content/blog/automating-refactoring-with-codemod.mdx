...
As more features are added into a product, the amount of code grows exponentially. By itself, this is not a problem when code are treated as a static artifacts that created once and run indefinitely. However, if we accept software entropy is inevitable and software rots without ongoing maintenance, then the huge amount of code will becomes a bottleneck of maintenance effort, just by the sheer amount of code you need to change when introducing a change of pattern.
...
Let's look at the actual steps to create a codemod for JavaScript.

Using [the previous article on TailwindCSS in Module Federation](/blog/using-tailwindcss-with-module-federation) as example, we want to transform usage of `twin.macro` to the new `tw` function call.
