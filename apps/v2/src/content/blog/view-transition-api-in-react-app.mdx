...
import { AnimatedDialog } from '@app/components/demo/animated-dialog';
import { ViewTransitionBlogOnboardingButton } from '@app/components/demo/onboarding';
import { TabsDemo } from '@app/components/demo/animated-tabs';
...
### React first-class support: ViewTransition component

As part of [React 19.3](https://react.dev/blog/2026/09/09/react-19-3), `ViewTransition` component is available from `React` directly.

<LiveEditor previewMinHeight="20rem" codeMaxHeight="20rem" readOnly fullBleed="when-preview" autoPlay>
  ...
  return (
    <div>
      <div className="top-bar">
        ...
        {isThumbnail && (
          // [!code highlight:1]
          <React.ViewTransition name="meow-image">
            <img
              src="https://res.cloudinary.com/djzsjzasg/image/upload/c_scale,w_300/v1678947391/malcolm-kee/meow_dtsn8h.png"
              alt="cat"
              className="thumbnail"
  ...
  return (
    <div>
      ...
      {displayedCats.length === 0 && (
        <React.ViewTransition name="error-message">
      ...
      <ul className="cat-list">
        {displayedCats.map((cat) => (
          <li key={cat.id}>
            {/* [!code highlight:1] */}
            <React.ViewTransition name={`cat-${cat.id}`}>
...
### Shared elements must be in the viewport

React only forms a shared-element pair when both the element being removed and the element being inserted are inside the viewport. If either side is outside the viewport, React treats each side as a regular enter or exit instead. This prevents an element from unexpectedly flying into or out of the screen when the page is scrolled.

This often shows up in a listing-to-detail transition. Try opening the first cat without scrolling: the thumbnail and detail hero are both visible, so React forms a shared-element transition between them. Then go back, scroll to the last cat, and open it. The detail page keeps the scroll position, leaving its hero image above the viewport, so React does not pair the two images.

<LiveEditor previewMinHeight="28rem" codeMaxHeight="28rem" readOnly autoPlay fullBleed="when-preview">
...
const App = () => {
  ...
  const openCat = (catId) => {
    React.startTransition(() => setSelectedCatId(catId));
  };

  const goBack = () => {
    React.startTransition(() => setSelectedCatId(null));
  };

  return (
    <main>
      <header className="demo-toolbar">
        {selectedCat ? <button onClick={goBack}>← Back to cats</button> : <b>Cat directory</b>}
        {selectedCat ? (
          <small>The detail hero is at the top; your scroll position was preserved</small>
        ) : (
          <small>Try the first card, then scroll down and try the last one</small>
        )}
      </header>
      ...
      {selectedCat ? (
        <article className="cat-detail">
          <div className="detail-header">
            ...
            <React.ViewTransition name={`cat-image-${selectedCat.id}`}>
              <img src={selectedCat.image} alt={selectedCat.name} className="detail-image" />
            </React.ViewTransition>
          </div>
          <div className="detail-copy">
            <h2>About {selectedCat.name}</h2>
            <p>
              This detail page is intentionally tall, like a real product or profile page. That lets
              it preserve the listing's scroll position during navigation.
            </p>
          </div>
        </article>
      ) : (
        <ul className="cat-list">
          {cats.map((cat) => (
            <li key={cat.id}>
              <button className="cat-card" onClick={() => openCat(cat.id)}>
                <React.ViewTransition name={`cat-image-${cat.id}`}>
...
This viewport check applies specifically to a named `share` transition where one component is removed and another is inserted. If the same component instance changes position, React classifies it as an `update`, and it can still animate even when one position is outside the viewport. See the [`ViewTransition` documentation](https://react.dev/reference/react/ViewTransition#animating-enterexit-with-activity) for the full caveat.
