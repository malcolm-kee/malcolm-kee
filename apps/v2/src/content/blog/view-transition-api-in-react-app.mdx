...
pubDate: 17 Mar 2023
description: Discover how to use the View Transition API in your React applications to create engaging and fluid navigational experiences. This API simplifies the process of animating shared elements and can be used in various practical scenarios, such as showcasing product images, animating modals, and more. Learn how to integrate the API with React and React Router, as well as how to handle browser compatibility with progressive enhancement techniques.
topics:
  - react
  - css
...
1. When `document.startViewTransition` is called, it

   - takes a screenshot of the entire page
   - finds all the elements with `view-transition-name` CSS property declared, and take a screenshot of it
...
Because React renders state changes asynchronously, we need to wrap the state-setter function with [`flushSync`](https://beta.reactjs.org/reference/react-dom/flushSync) to force the state changes to be applied synchronously.
...
1. Showcasing product images or gallery items: When a user clicks on a thumbnail, the View Transition API can be used to smoothly transition to the full-size image or gallery view.
...
1. Tab active indicator animation: When switching tab, animating the active indicator used to requiring manual computation of the position of the previous active tab versus the new active tab. With View Transition API we can delegates to browser to perform the computation.
