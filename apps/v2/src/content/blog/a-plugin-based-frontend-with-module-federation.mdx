...
With components coming from multiple remotes co-exists in single screen, it becomes inevitable sometimes that they want to communicate with each other. `usePluginEventEmitter` and `usePluginEventListener` are custom hooks that allows them to emit events and listen for events.
