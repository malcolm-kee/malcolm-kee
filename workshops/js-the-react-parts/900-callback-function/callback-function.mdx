...
Thanks to the fact that [function is first class in JavaScript](/functional-programming/#first-class-function), we can pass function as parameter. With that, we can create a function that has different behavior based on the function parameter.
...
Besides creating JavaScript function with dynamic behavior, callback function is also a [common pattern in asynchronous programming](/asynchronous-js/#asynchronous-programming-with-callback).
