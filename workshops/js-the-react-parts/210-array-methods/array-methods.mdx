...
Note: `map` use callback function as parameter. If this pattern is new to you, have a read at [callback function](/callback-function/).
...
Since these are so common, the group that maintain the JavaScript (TC39) decided to include them in JavaScript as part of the methods of `Array` object.
...
1. Given the following numbers:
   ...
   ```js
   const numbers = [3, -2, 1000, 201, 50, 33, -21];
   ```
...
`reduce` doesn't dictate the type of the `accumulator`, thus you can use it for many purposes:
...
const numberGroups = numbers.reduce(
  (result, num) => {
    if (num % 2 === 0) {
      // it's an even number, concat the item to the `even` array
      return {
        ...result,
        even: result.even.concat(num)
      };
    } else {
      // concat the item to the `odd` array.
      return {
        ...result,
        odd: result.odd.concat(num)
      };
    }
  },
  { odd: [], even: [] }
