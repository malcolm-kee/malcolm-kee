...
Evan You, the creator of Vue, shared [his thoughts](https://twitter.com/youyuxi/status/1175433025179529216) on how TypeScript actually reduces the use of `class` when Vue migrates to TypeScript.
...
Let's discuss briefly about how your code in TypeScript is being compiled (or transpiled/transformed) to the JavaScript that could actually run in browser.

The process is like how our React code in JSX is being transpiled to JavaScript with Babel.

A typical React project compilation pipeline look like this:

![Babel compile JSX by going through two process: parse and emit](babel-compilation.png)
...
![TypeScript compilations takes three steps: parse, typecheck, and emit](typescript-compilation.png)
...
![Babel able to parse TypeScript code and emit code, while TypeScript compiler is used to parse and typecheck, without emit code](babel-compilation-with-typescript-typecheck.png)
