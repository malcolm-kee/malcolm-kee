...
Gatsby has rich plugin libraries that may already able to source the data source that you need, such as [Wordpress](https://www.gatsbyjs.org/packages/gatsby-source-wordpress/), [Shopify](https://www.gatsbyjs.org/packages/gatsby-source-shopify), and [MySQL Database](https://www.gatsbyjs.org/packages/gatsby-source-mysql) (which is maintained by yours truly).
...
- resources that your user probably need very soon, such as the contents of the page whose link is currently seen by user will be retrieved in the background. This allows your page respond instantly when user interact with your page. (This technique is known as prefetching).
