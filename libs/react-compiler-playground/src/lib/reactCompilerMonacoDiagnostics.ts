...
import { CompilerDiagnostic, CompilerErrorDetail } from 'babel-plugin-react-compiler';
...
export function renderReactCompilerMarkers({
  ...
  for (const detail of details) {
    const marker = mapReactCompilerDiagnosticToMonacoMarker(detail, monaco, source);
    if (marker == null) {
      continue;
    }
    markers.push(marker);
